      ...
      <div class="col-md-4">
        <div class="order-details">
          ...
          if(isset($_REQUEST['wishlist_id'])){
            $id=$_REQUEST['wishlist_id'];
            $delete_wishlist=mysqli_query($con,"DELETE FROM `wish_list` WHERE proudect_id='$id'");
            }
              $user_email_email=$_SESSION['USER_ID'];
              $sql_wish_list="SELECT wish_list.id,wish_list.proudect_id,wish_list.user_id, product.name,product.images,product.price,product.mrp,product.id FROM wish_list INNER JOIN product on product.id=wish_list.proudect_id and wish_list.user_id='$user_email_email'";
              $sql_callect_wish_list=mysqli_query($con,$sql_wish_list);
              $total_price=0;
              while($get_proudect=mysqli_fetch_assoc($sql_callect_wish_list)){
              $pname=$get_proudect['name'];
              $pprice=$get_proudect['price'];
              $pimages=$get_proudect['images'];
              $pmrp=$get_proudect['mrp'];
              $proudect_wishlist_id=$get_proudect['id'];
          ...
          <div class="order-details__item">
            <div class="single-item">
              ...
              <div class="single-item__remove">
                <a href="?wishlist_id=<?php echo $proudect_wishlist_id?>"><i class="zmdi zmdi-delete"></i></a>
              </div>
            </div>
          </div>
